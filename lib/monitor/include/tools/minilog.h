...
#undef MINILOG_NAME_TO_LEVEL_FUNCTION
    ...
    inline auto
    output_log(log_level lev, std::string msg, std::source_location const &loc)
        -> void {
        ...
        if (lev >= details::g_log_level) { std::cout << msg; }

        if (details::g_log_file) { details::g_log_file << msg; }
    }
